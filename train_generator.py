"""Generator (Theme Transformer) Training Code
...
from discriminator import TransformerDiscriminator, Pitch_Discriminator, Rhythm_Discriminator
...
if args.multi_discriminator:
    ...
    pitch_discriminator.load_state_dict(torch.load('ckpts/debug_pitch/Discriminator_loss_best.pt',map_location=device_str))
    
    rhythm_discriminator = Rhythm_Discriminator(myvocab)
    rhythm_discriminator.to(device)
    rhythm_discriminator.load_state_dict(torch.load('ckpts/debug_rhythm/Discriminator_loss_best.pt',map_location=device_str))
...
def train(epoch_num):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        softmax_output = F.gumbel_softmax(output, hard=True)
        
        # Softargmax trick,详情见链接https://www.zhihu.com/question/422373907
        values = torch.arange(softmax_output.shape[-1]).to(device)
        # result_softarg就是可以使用Discriminator中进行辨别的数据格式了
        result_softarg = torch.Tensor.sum(softmax_output * values, dim=-1)
        result_softarg = torch.round(result_softarg).to(torch.int)
        result_softarg = result_softarg.permute(1, 0)
        # # 计算出本次输出的Discriminator分数
        discriminator_score = discriminator.discriminate_forward(softmax_output)

        loss_discriminator = F.cross_entropy(discriminator_score, torch.zeros(result_softarg.shape[0]).long().to(device))


        loss_regular = criterion(output.view(-1, myvocab.n_tokens),
                         fullsong_output.reshape(-1))
        
        loss_total = loss_regular + loss_discriminator*0.1
        # loss_total = loss_discriminator
        ...
        print("Loss : {:.2f} Loss_Dis : {:.2f} lr:{:.4f} ".format(
            loss_regular.item(), loss_discriminator, optimizer.param_groups[0]['lr']), end='\r')

        steps += 1
        train_step += 1

    mylogger.log("Epoch {} lr:{:.4f} train_acc : {:.2f} train_loss : {:.2f} discriminator_loss : {:.2f} time:{:.2f} ".format(epoch_num,
                                                                                                  optimizer.param_groups[0]['lr'], total_acc/steps, total_loss_regular/steps, total_loss_discriminator/steps, time.time()-start_time), end='')
    
    train_loss_regular = total_loss_regular/steps
    train_acc = total_acc/steps
    train_loss_discriminator = total_loss_discriminator/steps
    

    return train_loss_regular, train_loss_discriminator, train_acc
...
best_train_loss_discriminator = 1e100

for i in range(start_epoch,max_epoch):
    model.to(device)
    train_loss, train_loss_discriminator, train_acc = train(i)
    eval_acc, eval_loss = evalulate(i)
    model.to(device_cpu)
    ...
    writer.add_scalar('loss/train_loss_discriminator', train_loss_discriminator, i)
    ...
    if train_loss_discriminator < best_train_loss_discriminator:
        best_train_loss_discriminator = train_loss_discriminator
        torch.save(model.state_dict(), os.path.join(
            train_checkpoint_folder, "model_loss_discriminator_best.pt"))
